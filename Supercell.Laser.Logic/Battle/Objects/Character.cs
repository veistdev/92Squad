namespace Supercell.Laser.Logic.Battle.Objects
{
    ...
    public class Character : GameObject
    {
        ...
        public void SetGain(int time, int percentage)
        {
            m_gain = new Gain(time, percentage);
        }

        public void SetSpeedBoost(int time, int speedBoost)
        {
            m_speedBoost = new Speed(time, speedBoost);
        }

        public void SetStun(int time)
        {
            m_stun = new Stun(time);
        }
        ...
        public void ApplyItem(Item logicItem)
        {
            ...
            if (logicItem.ItemData.Name == "DamageAndSpeed"){
                SetGain(100, 10);
            }
        ...
        private void TickTimers()
        {
            ...
            if (m_speedBoost != null) 
            {
                if (m_speedBoost.Tick(1))
                {
                    m_speedBoost.Destruct();
                    m_speedBoost = null;
                }
            }
        ...
        private void TickBot()
        {
            ...
            ActivateSkill(r.Next(5) == 0, deltaX, deltaY);
        }
        ...
        private void HandleMoveAndAttack()
        {
            ...
            if (m_isMoving && !IsChargeActive())
            {
                if (Position.GetDistance(m_movementDestination) != 0)
                {
                    ...
                    if (m_speedBoost != null) movingSpeed += m_speedBoost.GetSpeedBoost();
                    if (m_gain != null) movingSpeed += 12;
                    if (m_stun != null) movingSpeed = 0;
        ...
        {
            if (m_gain != null) return WeaponSkillData.Damage + ((int)(((float)5 / 100) * (float)WeaponSkillData.Damage)) * (m_heroLevel + m_damageMultiplier + m_gain.GetGainBoost());
        ...
        {
            if (m_gain != null) return damage + ((int)(((float)5 / 100) * (float)damage)) * (m_heroLevel + m_damageMultiplier + m_gain.GetGainBoost());
        ...
        public override void Encode(BitStream bitStream, bool isOwnObject, int visionTeam)
        {
            ...
            if (m_speedBoost != null){
                bitStream.WriteBoolean(m_speedBoost.GetSpeedBoost() >= 0); // Speed up (Arrows up)
                bitStream.WriteBoolean(m_speedBoost.GetSpeedBoost() < 0); // Speed up (Arrows up)
            }
            else{
